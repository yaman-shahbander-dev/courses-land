...
class AuthorRequestSeeder extends Seeder
{
    /**
    ...
    public function run()
    {
        if(Schema::hasTable('authors_requests')){
            $faker = Faker::create();
            $users = User::all();
            $mycount = User::count();
            $loop = 0; //to help to determine where to start to make user do request to be an author
