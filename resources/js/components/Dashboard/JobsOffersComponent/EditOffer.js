...
const UpdateInfo = (offerID, title, description, picture) => {
    ...
    axios.post('http://127.0.0.1:8000/api/UpdateOfferInforamtion', data).then((res) => {
        if (res.data.error) {
            swal("Oh Sorry!", "Job offer doesn\'t exist!", "error");
        } else {
            swal("Well done!", "Job offer updated successfully!", "success");
        }
    });
}
...
const EditOffer = () => {
    ...
    useEffect(() => {
        axios.get(`http://127.0.0.1:8000/api/JobOfferExists?jobOfferID=${OfferID}&operation=${'edit'}`).then((res) => {
            setOfferID(res.data.success.id)
                            ...
                            <div className="col-lg-12 col-md-12 col-sm-12">
                                <div className="card">
                                    <div className="header">
                                        <h2><strong>Edit Job Offer</strong> {title}</h2>
                                    </div>
                                    <div className="body">
                                        <form>
                                            ...
                                            <button onClick={() => UpdateInfo(offerID, title, description, picture)} type="button" className="btn btn-raised btn-primary btn-round waves-effect">Update</button>
                                        </form>
                                    </div>
                                </div>
                            </div>
