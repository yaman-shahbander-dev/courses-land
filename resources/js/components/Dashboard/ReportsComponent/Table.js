...
const Table = () => {
    ...
    useEffect(() => {
        axios.get('http://127.0.0.1:8000/api/getReports').then((res) => setReports(res.data))
    }, []);

    const DeleteReport = (ReportID, operation) => {
        axios.get(`http://127.0.0.1:8000/api/reportExists?reportID=${ReportID}&operation=${operation}`)
            .then((res) => {
                if (res.data.success) {
                    ...
                    <div className="row">
                        <div className="col-lg-12 col-md-6 col-sm-12">
                            ...
                            <div className="form-group">
                                ...
                                <Link to={`searchReport?value=${searchValue}`} className="btn btn-primary btn-icon right_icon_toggle_btn" type="button"><i className="zmdi zmdi-arrow-right"></i></Link>
                            </div>
                        </div>
                    </div>
                </div>
